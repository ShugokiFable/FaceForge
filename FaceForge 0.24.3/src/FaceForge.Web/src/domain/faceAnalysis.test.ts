...
describe("face proportion mapping", () => {
  ...
  it("rebuilds stylized and unstable analyses instead of refining bad values", () => {
    const clean = createNeutralAnalysis();
    clean.correction.pairedLandmarks = 300;
    expect(assessAnalysisReliability(clean, false, 0.8, {})).toEqual({
      mode: "refine",
      reasons: []
    });

    expect(assessAnalysisReliability(clean, true, 0.8, {}).mode).toBe("interpret");

    const turned = createNeutralAnalysis();
    turned.correction.asymmetryBefore = 26;
    turned.correction.pairedLandmarks = 155;
    const verdict = assessAnalysisReliability(turned, false, 0.8, {});
    expect(verdict.mode).toBe("interpret");
    expect(verdict.reasons).toEqual(
      expect.arrayContaining([
        expect.stringContaining("disagree"),
        expect.stringContaining("Too few")
      ])
    );
  });
});
...
/**
 * The brow and iris baselines are guesses -- the neutral Skyrim head paints its brows and irises
 * on as textures, so a render has no geometry to measure. That was known and documented; what was
 * missed is that a guessed divisor was still handed full-range authority. On the export that
 * exposed it, every *measured* baseline put a normal face within 10% of neutral while every
 * *guessed* one threw it to the end of the range (EFM_Brow_Thickness 2.93 of 3.00, from a raw
 * 6.66 the saturation curve folded over).
 */
describe("estimated baselines have limited authority", () => {
  const exaggerated = (key: string, factor: number) => {
    const analysis = createNeutralAnalysis();
    const measurement = analysis.measurements[key as keyof typeof analysis.measurements];
    analysis.measurements[key as keyof typeof analysis.measurements] = {
      ...measurement,
  ...
  const valueOf = (groups: ReturnType<typeof generateEfmSliders>, name: string) =>
    groups.flatMap((group) => group.sliders).find((slider) => slider.name === name)?.value;

  it("caps a slider whose baseline is still a guess", () => {
    // lipGap keeps its estimate: 20 rendered heads disagree by 169% of the mean.
    const value = valueOf(generateEfmSliders(exaggerated("lipGap", 4)), "EFM_Oral_Height");
    expect(value).toBeDefined();
    // Rounded to two decimals on the way out, so compare against the cap at the same precision.
    expect(Math.abs(value!)).toBeLessThanOrEqual(
      Math.round(EFM_RANGE * ESTIMATED_BASELINE_AUTHORITY * 100) / 100
    );
  });

  it("still lets a measured-baseline slider use its full range", () => {
    const value = valueOf(generateEfmSliders(exaggerated("jawWidth", 1.6)), "EFM_Jaw_Width");
    expect(value).toBeDefined();
    expect(Math.abs(value!)).toBeGreaterThan(EFM_RANGE * ESTIMATED_BASELINE_AUTHORITY);
  });

  it("keeps the direction of a guessed-baseline measurement", () => {
    const up = valueOf(generateEfmSliders(exaggerated("lipGap", 2)), "EFM_Oral_Height");
    const down = valueOf(generateEfmSliders(exaggerated("lipGap", 0.4)), "EFM_Oral_Height");
    expect(up!).toBeGreaterThan(down!);
  });

  /**
   * 0.24.0 had all five here. Three were calibratable all along -- the rendered numbers were in
   * qa/race-calibration.json and pass 0.19.0's own "reject only if the heads disagree by more than
   * half the mean" test at 4.0%, 8.5% and 24.3%. Adopting them removed guesses that were 40%, 63%
   * and 30% low. Only the two that genuinely fail the rule remain.
   */
  it("keeps only the baselines that could not be measured", () => {
    // browAngle is uncalibrated too but divides degrees instead of a baseline, so it cannot be
    // distorted by one; it is deliberately absent.
    expect([...ESTIMATED_BASELINES].sort()).toEqual(["irisSize", "lipGap"]);
  });

  it("gives the newly measured brow baselines full authority", () => {
    for (const key of ["browHeight", "browWidth", "browThickness"] as const) {
      expect(ESTIMATED_BASELINES.has(key)).toBe(false);
    }
    // The guess that produced EFM_Brow_Thickness 2.93 was 0.03; the render says 0.0807.
    expect(measurementBaselines.browThickness).toBeCloseTo(0.0807, 4);
    expect(measurementBaselines.browWidth).toBeCloseTo(0.3013, 4);
    expect(measurementBaselines.browHeight).toBeCloseTo(0.1077, 4);
  });

  /**
   * The export that started this measured brow thickness at 0.0804. Against the old guess that was
   * +168% and pinned the slider; against the rendered baseline it is a face with ordinary brows.
   */
  it("turns the reported export's brow readings into ordinary values", () => {
    const analysis = createNeutralAnalysis();
    const set = (key: "browHeight" | "browWidth" | "browThickness", value: number) => {
      analysis.measurements[key] = { ...analysis.measurements[key], value };
    };
    set("browHeight", 0.1277);
    set("browWidth", 0.3209);
    set("browThickness", 0.0804);
    const sliders = sliderRecord(generateEfmSliders(analysis));
    expect(sliders.EFM_Brow_Height).toBeGreaterThan(0.5);
    expect(sliders.EFM_Brow_Height).toBeLessThan(1.2);
    expect(Math.abs(sliders.EFM_Brow_Width)).toBeLessThan(0.6);
    expect(Math.abs(sliders.EFM_Brow_Thickness)).toBeLessThan(0.1);
  });
});

describe("trust reporting tells the truth about what was kept", () => {
  it("does not call a partially faded measurement neutral", () => {
    // jawOpen fades the mouth family without zeroing it.
    const analysis = measureFace(syntheticFace(), { jawOpen: 0.75 }, 1);
    const faded = Object.keys(analysis.trust ?? {}).filter(
      (key) => (analysis.trust![key as keyof typeof analysis.trust] ?? 1) < 0.35
    );
    if (faded.length === 0) return; // fixture no longer trips the rule; nothing to assert
    const stillContributing = faded.filter(
      (key) => (analysis.trust![key as keyof typeof analysis.trust] ?? 1) > 0.001
    );
    if (stillContributing.length === 0) return;
    const claimedNeutral = analysis.warnings.find((line) =>
      line.includes("left at the neutral default")
    );
    for (const key of stillContributing) {
      expect(claimedNeutral ?? "").not.toContain(key);
    }
    expect(analysis.warnings.join(" ")).toContain("still contribute");
  });

  it("still reports a genuinely neutral measurement as neutral", () => {
    const analysis = measureFace(syntheticFace(), {}, 1, { forehead: 1 });
    expect(analysis.warnings.join(" ")).toContain("left at the neutral default");
    expect(analysis.warnings.join(" ")).toContain("hair over the forehead");
  });

  it("stops a covered forehead from railing the brow sliders", () => {
    const covered = generateEfmSliders(measureFace(syntheticFace(), {}, 1, { forehead: 1 }));
    const brow = covered
      .flatMap((group) => group.sliders)
      .filter((slider) => slider.name.startsWith("EFM_Brow_"));
    expect(brow.length).toBeGreaterThan(0);
    for (const slider of brow) {
      expect(Math.abs(slider.value)).toBeLessThan(0.05);
      expect(slider.atLimit).toBeUndefined();
    }
  });
});
