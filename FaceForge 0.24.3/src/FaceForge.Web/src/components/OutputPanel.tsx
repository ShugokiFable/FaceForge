...
import { EFM_PLUGIN, type RaceMenuTemplate } from "../domain/racemenu";
...
export default function OutputPanel({
  ...
  // EFM is not just one dependency among several: it is the only slider family
  // FaceForge writes, so its absence voids the whole export rather than degrading it.
  const efmMissing = dependencies.some(
    (dependency) => dependency.pluginName === EFM_PLUGIN && !dependency.present
  );
  ...
  return (
    <section className="workspace-panel output-panel">
      ...
      {/* Every slider FaceForge writes belongs to the Expressive Facegen Morphs family.
          Without that plugin active in the game, RaceMenu silently ignores all of them
          and the character stays at the race default -- which is why two very different
          photographs can both come out as the same generic face with no error anywhere.
          It has to be said here, next to the export, not only as a dot in a list. */}
      {dependencyIndexed && efmMissing ? (
        <div className="dependency-card warning-card">
          <h3>Expressive Facegen Morphs is not installed</h3>
          <p>
            FaceForge writes only the <strong>EFM_</strong> slider family, so without that
            plugin enabled in Skyrim, RaceMenu ignores every value in this preset. The
            character will load as the plain race default and look the same no matter which
            photo you used. Install <strong>Expressive Facegen Morphs</strong>, enable it,
            then load the preset again.
          </p>
        </div>
      ) : null}
            ...
            <div className="dependency-list">
              {dependencies.map((dependency) => (
                <div className="dependency-row" key={dependency.pluginName}>
                  ...
                  <small>
                    {/* A plugin can be found while the mod that supplies it cannot be
                        named. Printing "missing" for that case contradicted the
                        "n/n present" counter directly above it. */}
                    {dependency.baseGame
                      ? "base game"
                      : dependency.sourceMod
                        ?? (dependency.present
                          ? "present, source mod not identified"
                          : dependencyIndexed ? "missing" : "unresolved")}
                  </small>
                </div>
              ))}
            </div>
              ...
              {isOpen && (
                <div className="slider-group-body">
                  {group.sliders.map((slider) => {
                    ...
                    // "neutral" must mean the value really is the neutral default. A measurement
                    // faded to 30% confidence still exports 30% of its deviation, and labelling
                    // that "neutral" beside a slider reading 0.46 is simply false.
                    const held = slider.confidence <= 0.001;
                    const reduced = !held && slider.confidence < 0.9;
                    const detail = held
                      ? `Source: ${slider.source} — the photo could not measure this at all, so it is left at the neutral default. Set it by hand if you know the shape.`
                      : reduced
                        ? `Source: ${slider.source} — measured at ${Math.round(slider.confidence * 100)}% confidence; it keeps that share of its deviation and still contributes.`
                        : `Source: ${slider.source}`;
                    const estimatedNote = slider.estimated
                      ? " This axis has no rendered reference to measure against — Skyrim paints brows and irises on as textures — so its value is a deviation from an estimate, deliberately kept gentle."
                      : "";
                    const limitNote = slider.atLimit
                      ? " The value is set by that limit, not by the measurement."
                      : "";
                    return (
                      <label
                        className={`slider-row${held ? " held" : reduced ? " reduced" : ""}`}
                        key={slider.name}
                        title={`${detail}${estimatedNote}${limitNote}`}
                      >
                        <span>
                          {slider.label}
                          ...
                          {slider.estimated ? (
                            <em className="slider-flag estimated">estimated</em>
                          ) : null}
                          {slider.atLimit ? (
                            <em className="slider-flag at-limit">at limit</em>
                          ) : null}
                        </span>
                        <input
                          type="range"
