...
public static class MorphRegistry
{
    ...
    public static MorphRegistrySnapshot Build(ResolvedDataView view)
    {
        ...
        // A mod may register its morphs from an ini packed inside its own archive, which this
        ...
        foreach (var directory in view.ChildDirectories(MorphsRoot))
        {
            ...
            foreach (var (_, ini) in view.Files(directory, "morphs.ini", recursive: false))
            {
                foreach (var line in ReadIniLines(ini))
                {
                    ...
                    // One line registers ONE base mesh against ANY NUMBER of extension .tri files:
                    //   extension = <base>, <ext1>, <ext2>, ...
                    // High Poly Head puts nine on a single line (cme, ece, rans, nuska, extra,
                    // expr, efm, race). Reading only the first extension loses the other eight.
                    ...
                    foreach (var extension in parts.Skip(1))
                    {
                        ...
                        results.Add(new MorphExtensionRegistration(
                            plugin, Normalize(parts[0]), Normalize(extension)));
                    }
                }
            }
        }
