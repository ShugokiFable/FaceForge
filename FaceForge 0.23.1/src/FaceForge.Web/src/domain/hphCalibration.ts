 ...
 *    defined by the TRI morph it drives. Nothing measured shows a given EFM value moving an HPH
 *    head less than a vanilla one; the claim was inferred from author *output*, not from the mesh.
 *
 *    (The second half of that sentence used to read "and High Poly Head ships its own
 *    EFM-compatible morphs". Measured 2026-08-05 and false on this installation: Expressive
 *    Facegen Morphs registers its extension against the vanilla
 *    Actors\Character\Character Assets head and ships 996-vertex morphs, while the HPH head is
 *    3832. Nothing registers an EFM extension against the KL path, so those sliders are inert on
 *    HPH rather than weaker -- which is a gating question, not a gain question, and is handled in
 *    sliderCatalog/MorphRegistry. It does not change the conclusion below.)
