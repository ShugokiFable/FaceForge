...
describe("estimated baselines have limited authority", () => {
  ...
  it("caps a guessed-baseline slider well short of the family limit", () => {
    // 2.68x is what the real export implied for brow thickness (0.0804 against a guessed 0.03).
    const value = valueOf(generateEfmSliders(exaggerated("browThickness", 2.68)), "EFM_Brow_Thickness");
  ...
  it("keeps the direction of a guessed-baseline measurement", () => {
    const up = valueOf(generateEfmSliders(exaggerated("browHeight", 1.4)), "EFM_Brow_Height");
    const down = valueOf(generateEfmSliders(exaggerated("browHeight", 0.6)), "EFM_Brow_Height");
    expect(up!).toBeGreaterThan(0);
    expect(down!).toBeLessThan(0);
  });

  it("covers every baseline the source marks estimated", () => {
    // browAngle is estimated too but divides degrees instead of a baseline, so it cannot be
    // distorted by one; it is deliberately absent.
    expect([...ESTIMATED_BASELINES].sort()).toEqual(
      ["browHeight", "browThickness", "browWidth", "irisSize", "lipGap"].sort()
    );
  });
});
