 ...
 */
/**
 * The baselines below that are a guess rather than a render measurement, and the authority a
 * guess is allowed to have.
 *
 * A baseline is a divisor: the slider is `(measured / baseline - 1) * sensitivity * gain`. When
 * the divisor is small and wrong, the error is not a small bias -- it is a multiplier. Measured
 * against the real numbers from the same export, the difference is not subtle:
 *
 *     EFM_Nose_Width      -0.45   -7% off baseline    baseline measured
 *     EFM_Jaw_Width       -0.58  -10% off baseline    baseline measured
 *     EFM_Brow_Height      2.30  +70% off baseline    baseline GUESSED (0.075)
 *     EFM_Brow_Width       2.43  +78% off baseline    baseline GUESSED (0.18)
 *     EFM_Brow_Thickness   2.93 +168% off baseline    baseline GUESSED (0.03)
 *
 * Every measured baseline lands a normal face inside 10% of neutral. Every guessed one throws it
 * to the far end of the range -- not for this photograph, for any photograph, because 0.03 is
 * simply not what a brow measures. The saturation curve then hides it: 2.93 looks like a strong
 * reading rather than a raw 6.66 folded over.
 *
 * The honest fix is not a better guess. The comment on measurementBaselines already explains why
 * these cannot be measured -- Skyrim's brows and irises are textures on plain geometry, so a
 * render of the neutral head has no brow to measure and calibrating against the landmarks it
 * returns anyway would "trade a guess for a confidently wrong number". That reasoning was right
 * and the numbers stayed estimated; what was missed is that an estimate was still handed the same
 * full-range authority as a measurement.
 *
 * So a guessed baseline keeps its direction and its ordering between faces, and loses its claim to
 * sculpt hard. 0.3 of the family range puts it near the mean |value| of hand-authored presets
 * (0.79 on EFM's +/-3) instead of at their ceiling.
 */
...
export const measurementBaselines: Record<MeasurementKey, number> = {
  ...
  lowerLidCurve: 0.123,
  eyeTilt: 0.0,
  irisSize: 0.115, // estimated: iris is an eye texture, not geometry
  browThickness: 0.03, // estimated: no brow geometry on the neutral mesh
  lipFullness: 0.1585,
  lipGap: 0.012, // estimated: near zero on a closed neutral mouth; the heads disagreed by 175% of the mean
  faceAspect: 1.1407,
  cheekWidth: 0.8736,
  cheekHeight: 0.4854,
  jawWidth: 0.8237,
  ...
  browHeight: 0.075, // estimated: no brow geometry on the neutral mesh
  browAngle: 0.0, // estimated: no brow geometry on the neutral mesh
  browWidth: 0.18, // estimated: no brow geometry on the neutral mesh
  noseWidth: 0.2304,
  ...
  for (const definition of selectDefinitions(inventory, availability)) {
    ...
    const authority =
      definition.angleDivisor === undefined && ESTIMATED_BASELINES.has(definition.source)
        ? range * ESTIMATED_BASELINE_AUTHORITY
        : range;
    groups.get(definition.group)!.push({
      name: definition.name,
      label: definition.label,
      value: round(saturate(raw, authority)),
