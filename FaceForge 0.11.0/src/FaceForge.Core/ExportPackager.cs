...
public static class ExportPackager
{
            ...
            else
            {
                if (!request.PreserveSculpt)
                    throw new InvalidOperationException(
                        "Follower Forge Head Kit requires Preserve Sculpt so the JSlot matches the source NIF/DDS.");
                if (!File.Exists(request.SourceNifPath) || !File.Exists(request.SourceDdsPath))
                    throw new InvalidOperationException(
                        "Follower Forge Head Kit requires a matched CharGen NIF and DDS.");
                AddText(
                    archive,
                ...
                AddText(
                    ...
                    JsonSerializer.Serialize(
                        ...
                        {
                            schemaVersion = 2,
                            product = "FaceForge Follower Forge Head Kit",
                            name,
                            targetRaceEditorId = request.TargetRaceEditorId,
                            targetSex = request.TargetSex,
                            source = new
    ...
    private static string BuildHeadKitReadme(string name, ExportRequest request) => $"""
        FaceForge Follower Forge Head Kit: {name}
        ...
        Forge, place the three files where Follower Forge scans:
           Data\SKSE\Plugins\CharGen\Presets\{name}.jslot
           Data\SKSE\Plugins\CharGen\{name}.nif
           Data\SKSE\Plugins\CharGen\{name}.dds
        Follower Forge enumerates the NIF files in Data\SKSE\Plugins\CharGen and matches each to
    ...
    private static string BuildRaceMenuExportReadme(string name, ExportRequest request) => $"""
        ...
        This is a round trip. Skyrim itself has to bake the head, then Follower Forge picks it
        ...
        6. Open Follower Forge. It scans Data\SKSE\Plugins\CharGen for exactly that NIF/DDS pair
