...
Run `build.ps1`, then `package.ps1`. The result is exactly one shareable file: `FaceForge 0.21.1 - STANDALONE.exe`. The web app, MediaPipe model/runtime, .NET runtime, and native libraries are embedded. Microsoft Edge WebView2 Runtime remains the normal Windows prerequisite.
