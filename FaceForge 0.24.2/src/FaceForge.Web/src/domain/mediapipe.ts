...
import {
  sampleOcclusion,
  scoreOcclusion,
  type OcclusionReading,
  type Rgb
} from "./occlusion";

/**
 * Reads the forehead-occlusion score off the frame the landmarks were detected on. Returns null
 * whenever pixels cannot be read -- no 2D context, a tainted canvas, a headless test environment --
 * so the caller keeps its previous behaviour instead of acting on an invented reading.
 */
export function readOcclusion(
  image: HTMLImageElement | HTMLCanvasElement,
  result: FaceLandmarkerResult
): OcclusionReading | null {
  ...
  const read = (x: number, y: number): Rgb | null => {
    const px = Math.round(x * width);
    const py = Math.round(y * height);
    if (px < 0 || py < 0 || px >= width || py >= height) return null;
    const offset = (py * width + px) * 4;
    if (data.data[offset + 3] === 0) return null;
    return { r: data.data[offset], g: data.data[offset + 1], b: data.data[offset + 2] };
  };
  const samples = sampleOcclusion(read, landmarks);
  return samples ? scoreOcclusion(samples) : null;
}
...
export interface UprightAnalysis {
  ...
  /** Forehead-cover reading for the measured frame, or null when pixels could not be read. */
...
export async function analyzePortraitUpright(
  ...
  const fallback: UprightAnalysis = {
    ...
    occlusion: readOcclusion(source, original)
  };
  ...
  return {
    ...
    recoveryStrategy: attempt.strategy,
    candidateFaces: attempt.candidates,
    occlusion: readOcclusion(canvas, refined)
  };
}
